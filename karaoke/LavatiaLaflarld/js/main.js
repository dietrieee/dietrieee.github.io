...
function addList(data, type) {
  if (type == "now-singing") {
    $("#now-singing-list").append(
      `<div class='s-container'><div class="parent-title-now-singing"><div class="title-now-singing ">${data.value}</div></div><button onclick="edit(${data.id}, 'now-singing')"class="btn btn-action btn-light  action"><i class="icon-pencil"></i></button><button onclick="deleteSong(${data.id}, 'now-singing')"class="btn btn-light btn-action  action"><i class="icon-eraser"></i></button><button onclick="drop()"class="btn btn-light btn-action  action"><i class="icon-drop"></i></button></div>`
    );
  }
  if (type == "list") {
    $("#song-list").append(
      `<div class='s-container'><div class="parent-title"><div class="title ">${data.value}</div></div><button onclick="edit(${data.id}, 'list')"class="btn btn-action btn-light  action"><i class="icon-pencil"></i></button><button onclick="deleteSong(${data.id}, 'list')"class="btn btn-light btn-action  action"><i class="icon-eraser"></i></button><button onclick="play(${data.id})"class="btn btn-action btn-light  action"><i class="icon-play"></i></button></div>`
    );
  }
}
function drop() {
