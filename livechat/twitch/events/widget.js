...
const checkSub = (event) => {
  if (fieldData.subAggregate) {
    ...
    if (event.isCommunityGift) {
      event.sender = event.sender.toLowerCase();
      if (typeof commGifts[event.sender] === "undefined" && event.amount === 1)
        return true;
  ...
  if (
    ...
    event.amount >= fieldData.minimumSubDuration
  )
    return true; // Minimum duration check
  if (parseInt(event.tier) >= 2000 && fieldData.minimumSubTier === "t2")
    return true; // Tier 2 check
  if (parseInt(event.tier) === 3000 && fieldData.minimumSubTier === "t3")
    return true; // Tier 3 check
  return false;
};
  ...
  if (event.type === "subscriber") {
    ...
    if (event.gifted || event.bulkGifted) {
      if (event.bulkGifted) {
        addEvent(
          event.type,
          wrapText(fieldData["sub-community-text"], event),
          event.name,
          isHistorical
        );
      } else {
        addEvent(
          event.type,
          wrapText(fieldData["sub-gift-text"], event),
          event.name,
          isHistorical
        );
      }
      ...
      addEvent(
        event.type,
        wrapText(fieldData["sub-text"], event),
        event.name,
        isHistorical
      );
    }
    ...
    addEvent(
      event.type,
      wrapText(fieldData[`${event.type}-text`], event),
      event.name,
      isHistorical
    );
  } else {
...
const getBadges = (apiKey) => {
  return new Promise((resolve) => {
    fetch("https://api.streamelements.com/kappa/v2/channels/me", {
      ...
      .then((obj) => {
        fetch(
          `https://badges.twitch.tv/v1/badges/channels/${obj.providerId}/display`
        )
          .then((response) => response.json())
          .then((data) => {
            if (data.badge_sets.subscriber.versions) {
              Object.keys(data.badge_sets.subscriber.versions)
                .sort()
